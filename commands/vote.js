    ...
    await msg.channel.send("Hey " +  msg.author.toString() + "! Lets setup your vote. First of all:\n**What is your question?**").then(async setupMSG => {
        ...
        switch (possibleAnswers.length) {
            case 2:
                ...
                var filter = filter = (reaction, user) => (reaction.emoji.name === '1⃣' || reaction.emoji.name === '2⃣') && !(votes.includes(user.id));
                break;
            case 3:
                await voteMSG.react('1⃣');
                await voteMSG.react('2⃣');
                await voteMSG.react('3⃣');
                var filter = filter = (reaction, user) => (reaction.emoji.name === '1⃣' || reaction.emoji.name === '2⃣' || reaction.emoji.name === '3⃣') && !(votes.includes(user.id));
                break;
            case 4:
                await voteMSG.react('1⃣');
                await voteMSG.react('2⃣');
                ...
                var filter = filter = (reaction, user) => (reaction.emoji.name === '1⃣' || reaction.emoji.name === '2⃣' || reaction.emoji.name === '3⃣' || reaction.emoji.name === '4⃣') && !(votes.includes(user.id));
                break;
            case 5:
                await voteMSG.react('1⃣');
                await voteMSG.react('2⃣');
                ...
                var filter = filter = (reaction, user) => (reaction.emoji.name === '1⃣' || reaction.emoji.name === '2⃣' || reaction.emoji.name === '3⃣' || reaction.emoji.name === '4⃣' || reaction.emoji.name === '5⃣') && !(votes.includes(user.id));
                break;
        }
        var votesForOne   = 0;
        var votesForTwo   = 0;
        ...
        await voteMSG.awaitReactions(filter, { time: millisecs }).then(collected => {
            if (collected.get('1⃣') != undefined) {
                votesForOne = collected.get('1⃣').count - 1;
            }
            if (collected.get('2⃣') != undefined) {
                votesForTwo = collected.get('2⃣').count - 1;
            }
            if (collected.get('3⃣') != undefined) {
                votesForThree = collected.get('3⃣').count - 1;
            }
            if (collected.get('4⃣') != undefined) {
                votesForFour = collected.get('4⃣').count - 1;
            }
            if (collected.get('5⃣') != undefined) {
                votesForFive = collected.get('5⃣').count - 1;
            }
        });
        switch (possibleAnswers.length) {
            case 2:
